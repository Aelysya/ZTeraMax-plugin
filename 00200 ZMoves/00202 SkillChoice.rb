module BattleUI
  class SkillChoice
    ...
    def initialize(viewport, scene)
      @zmove_enabled = false
      default_initialize(viewport, scene)
    end
    ...
    class SubChoice
      ...
      alias default_action_y action_y
      def action_y
      ...
      def zmove_related_stuff(pokemon, zcrystal_activated)
        ...
        if zcrystal_activated
          pokemon.moveset.map! do |move|
            ...
            Battle::Move[:s_basic].new(ZCRYSTALS[pokemon.item_db_symbol][data_move(move.db_symbol).category], 1, 1, @scene)
          end
        else
